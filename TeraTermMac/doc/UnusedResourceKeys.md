...
| ~~ログを中断~~ | ~~ログの一時停止~~ → **実装済み** (`pauseLog()` + `TerminalLogger.pauseLogging()`) |
| ~~ログにコメントを付加~~ | ~~ログ中にコメント追記~~ → **実装済み** (`TerminalLogger.logComment()` + コメント入力ダイアログ) |
| ~~ログを表示~~ | ~~外部エディタでログ表示~~ → **実装済み** (`viewLog()` + NSWorkspace.open) |
| ~~ログ記録中ダイアログを表示~~ | ~~ログ進捗ダイアログ~~ → **実装済み** (`LogProgressPanel` + 一時停止/コメント/閉じるボタン) |
| ~~Tera Termの全終了~~ | ~~全ウインドウの強制終了~~ → **実装済み** (`quitAllTeraTerm()` + 確認ダイアログ) |

### 転送メニュー

| Windows版の機能 | 備考 |
|----------------|------|
| ~~YMODEM 送受信~~ | ~~プロトコル未実装~~ → **実装済み** (`YMODEMProtocol` + バッチ送信対応) |
| ~~B-Plus 送受信~~ | ~~プロトコル未実装~~ → **実装済み** (`BPlusProtocol`) |
| ~~Quick-VAN 送受信~~ | ~~プロトコル未実装~~ → **実装済み** (`QuickVANProtocol`) |
| ~~Kermit 取得 / Kermit 終了~~ | ~~サブコマンド未実装~~ → **実装済み** (`kermitGet()` / `kermitFinish()` + `KermitMode` enum) |

### 編集メニュー

| Windows版の機能 | 備考 |
|----------------|------|
| ~~貼り付け\<CR\> (`Alt+R`)~~ | ~~CR付き貼り付け~~ → **実装済み** (`pasteCR()`) |
| ~~選択を解除~~ | ~~選択範囲のキャンセル~~ → **実装済み** (`deselect()`) |
| ~~表示画面を選択~~ | ~~表示中の画面のみ選択~~ → **実装済み** (`selectScreen()`) |

### コントロールメニュー

| Windows版の機能 | 備考 |
|----------------|------|
| ~~リモートタイトルリセット~~ | ~~リモート設定タイトルのリセット~~ → **実装済み** (`resetRemoteTitle()`) |
| ~~TEKウィンドウを開く / 閉じる~~ | ~~TEK 4014エミュレーション~~ → **実装済み** (`toggleTEKWindow()` + `TEKWindowController`) |
| ~~マクロウィンドウの表示~~ | ~~マクロ実行ウインドウの表示切替~~ → **実装済み** (`showMacroWindow()` + `MacroStatusPanelController`) |
...
| Windows版の機能 | 備考 |
|----------------|------|
| ~~キーボード種類選択~~ | ~~VT100/VT200等の選択~~ → **実装済み** (`TerminalID` enum + キーボード設定ダイアログ) |
| ~~アプリケーションキーパッドの無効化~~ | ~~モード無効化オプション~~ → **実装済み** (`disableAppKeypad` + キーボード設定ダイアログ) |
| ~~アプリケーションカーソルの無効化~~ | ~~モード無効化オプション~~ → **実装済み** (`disableAppCursor` + キーボード設定ダイアログ) |
...
| ~~テーマ起動設定 (固定テーマ/ランダムテーマ)~~ | ~~テーマ自動選択~~ → **実装済み** (`startupTheme` + ThemeTab) |
...
| ~~ダイアログフォント設定~~ | ~~UI用フォント変更~~ → **実装済み** (`dialogFontName` / `dialogFontSize` + UITab フォント選択) |

### テーマエディタ

| Windows版の機能 | 備考 |
|----------------|------|
| ~~テーマプレビュー/読み込み/保存~~ | ~~テーマファイル管理~~ → **実装済み** (`ThemeTab` + `themeFile` / `themeEnabled` / カラーエディタ) |
...
| Windows版の機能 | 備考 |
|----------------|------|
| ~~印刷中ダイアログ~~ | ~~VTウインドウ印刷進捗~~ → **実装済み** (`NSPrintOperation.showsProgressPanel = true`) |
...
- **接続エラーメッセージ**: `error.connection.failed` 等は `ConnectionError` enum の `errorDescription` / `alertTitle` で `L()` マクロ経由でローカライゼーションキーを使用済みです。
